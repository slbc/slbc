...
Press Coders Theme Framework v0.1.1
...
class PC_Framework {
	...
	public function __construct($theme_name) {
		...
		$this->_theme_customizer_class		= new PC_Theme_Customizer();
	...
	public function setup() {
		...
		/* Use the default method to render the front page before content widget area, or use a custom method. */
		// @todo replace this if/else block and just have the add_action below. Then in the theme specific utility class try to remove the hook below and add a new one.
		//       then in a child theme try to undo THAT hook. This needs to work properly.
		if( method_exists( $this->_ts_utility_class, 'custom_front_page_before_content_location') ) {
			// @todo should add the action that is in this callback here. Don't really need a cb function just to add the action. It's a bit redundant.
			$this->_ts_utility_class->custom_front_page_before_content_location();
		}
		else {
	...
	public function framework_features() {
		...
		/* Initialise if theme options supported. */
		if( current_theme_supports( 'theme-options-page' ) ) {
            ...
            if( current_theme_supports( 'breadcrumb-trail' ) ) {
                ...
                add_action( 'pc_theme_option_defaults', array( &$this->_ts_utility_callbacks_class, 'set_breadcrumb_theme_option_defaults' ) );
			...
			if( current_theme_supports( 'color-schemes' ) ) {
				...
				add_action( 'customize_register', array( &$this->_theme_customizer_class, 'theme_customizer_register_color_schemes' ) );
				...
				if( is_array( $_wp_theme_features['color-schemes'] ) ) {
						...
						/* @todo Should use 'widgets_init' hook here rather than 'wp_enqueue_scripts' as set_cookie() needs to run before any HTML ouptut is sent? */
						add_action( 'wp_enqueue_scripts', array( &$this->_utility_callbacks_class, 'enqueue_color_scheme' ), 8 ); /* Enqueue the color scheme. */
				}
			}
			...
			/* @todo When all themes use the customizer version of Google web fonts then the code below and all callbacks it contains can be deleted. */
			if( current_theme_supports( 'google-fonts' ) ) {
				...
				add_action( 'wp_enqueue_scripts', array( &$this->_utility_callbacks_class, 'enqueue_google_font' ) );
			...
			if( current_theme_supports( 'seo' ) ) {

				add_action( 'pc_theme_option_defaults', array( &$this->_utility_callbacks_class, 'set_seo_theme_option_defaults' ) );
				add_action( 'pc_set_theme_option_fields_misc', array( &$this->_utility_callbacks_class, 'set_seo_theme_option_fields' ) );
			}

			/* Custom CSS editing. */
			if( current_theme_supports( 'custom-css' ) ) {

				add_action( 'pc_theme_option_defaults', array( &$this->_utility_callbacks_class, 'set_custom_css_defaults' ) );
				add_action( 'pc_set_theme_option_fields_1', array( &$this->_utility_callbacks_class, 'set_custom_css_option_fields' ) );
				add_action( 'pc_head', array( &$this->_utility_callbacks_class, 'pc_custom_wp_head' ) );
			}

			/* Homepage Bar. */
			if( current_theme_supports( 'homepage-bar' ) ) {

				add_action( 'pc_theme_option_defaults', array( &$this->_utility_callbacks_class, 'set_homepage_bar_defaults' ) );
				add_action( 'pc_set_theme_option_fields_1', array( &$this->_utility_callbacks_class, 'set_homepage_bar_option_fields' ) );
				add_action( 'pc_after_container', array( &$this->_utility_callbacks_class, 'render_homepage_bar' ) );
			}

			/* Insert code into WordPress header/hooter hook callback functions, and footer link HTML in the theme <footer> tag. */
			if( current_theme_supports( 'hf-code-insert' ) ) {

				add_action( 'pc_theme_option_defaults', array( &$this->_utility_callbacks_class, 'set_hf_code_insert_defaults' ) );
				add_action( 'pc_set_theme_option_fields_gs', array( &$this->_utility_callbacks_class, 'set_hf_code_insert_option_fields' ) );
				add_action( 'wp_head', array( &$this->_theme_options_class, 'theme_header_html' ) );
				add_action( 'wp_footer', array( &$this->_theme_options_class, 'theme_footer_html' ) );
				add_action( 'pc_before_closing_footer_tag', array( &$this->_utility_callbacks_class, 'render_footer_links' ) );
			}

			/* Custom logo uploader. */
			/* @todo Deprecate this function and all callbacks when all themes use the customizer to handle logo uploads. */
			if( current_theme_supports( 'custom-logo' ) ) {

				add_action( 'pc_theme_option_defaults', array( &$this->_utility_callbacks_class, 'set_custom_logo_defaults' ) );
				add_action( 'pc_theme_option_js', array( &$this->_utility_callbacks_class, 'set_theme_option_js_custom_logo' ) ); /* Insert JS into theme options page. */
				add_action( 'pc_set_header_theme_option_fields', array( &$this->_utility_callbacks_class, 'set_custom_logo_option_fields' ), 9 );
			}
		...
		/* Google web font options via the theme cusotmizer. */
		if( current_theme_supports( 'google-fonts-customizer' ) ) {

			/* @todo Add some of the code below to callback function(s). */
			global $pc_google_font_list;
			global $pc_default_google_font;
			global $pc_google_font_nice_list;

			$pc_google_font_nice_list = array(); /* Initialize empty font nice list. */
			$default_font_list = array( 'PT+Sans+Narrow:400,700', 'Droid+Sans:400,700', 'Droid+Serif:400,700,700italic,400italic', 'Oswald', 'Ubuntu:300,400,500,700,300italic,400italic,500italic,700italic', 'Anton', 'Arvo:400,700,400italic,700italic' );

			if( is_array( $_wp_theme_features['google-fonts-customizer'] ) ) {

				/* Check an array of fonts has been specified in functions.php. If not add some defaults. */
				if( !is_array( $_wp_theme_features['google-fonts-customizer'][0]['font_list'] ) ) {
					$_wp_theme_features['google-fonts-customizer'][0]['font_list'] = $default_font_list;
				}

				/* Make sure we have a default font. */
				if( !empty($_wp_theme_features['google-fonts-customizer'][0]['default']) ) {
					$pc_default_google_font = $_wp_theme_features['google-fonts-customizer'][0]['default'];
				}
				else {
					/* Set font defaults to first in font list array. */
					$pc_default_google_font = $_wp_theme_features['google-fonts-customizer'][0]['font_list'][0];
					$_wp_theme_features['google-fonts-customizer'][0]['default'] = $pc_default_google_font; /* Maintain integrity of array. */
				}

				/* Add default fonts to the main font list, if it wasn't added in functions.php. */
				if( $pc_default_google_font != "None" && !in_array($pc_default_google_font, $_wp_theme_features['google-fonts-customizer'][0]['font_list']) )
					$_wp_theme_features['google-fonts-customizer'][0]['font_list'][] = $pc_default_google_font;

				$pc_google_font_list = $_wp_theme_features['google-fonts-customizer'][0]['font_list'];
			}
			else {
				$pc_google_font_list = $default_font_list;
				$pc_default_google_font = $default_font_list[0];
			...
			/* Add the first option as 'None', to be able to disable Google web fonts. */
			$pc_google_font_nice_list['none'] = 'None';
			foreach( $pc_google_font_list as $pc_google_font ) {
				$pc_google_font_array = explode(":", $pc_google_font);
				$pc_google_font_txt = str_replace( '+', ' ', $pc_google_font_array[0] );
				$pc_google_font_nice_list[$pc_google_font] = $pc_google_font_txt;
			}

			/* Update font in the theme customizer defaults array. */
			global $pc_customizer_defaults;
			$pc_customizer_defaults['pc-google-webfonts'] = $pc_default_google_font;

			/* Add callbacks. */
			add_action( 'customize_register', array( &$this->_theme_customizer_class, 'theme_customizer_register_google_webfonts' ) );
			add_action( 'wp_enqueue_scripts', array( &$this->_theme_customizer_class, 'enqueue_customizer_google_font' ), 1 ); // Make it one of the first to be enqueued
			add_action( 'pc_head', array( &$this->_theme_customizer_class, 'theme_customizer_google_fonts_css' ), 11 );
		}
        ...
        if( current_theme_supports( 'custom-post-types' ) ) {
            ...
            if( is_array( $_wp_theme_features['custom-post-types'] ) ) {
				...
				if( is_array($custom_post_types) ) {
					...
					/* Add Support CTP if specified. */
					if( in_array( 'support', $custom_post_types ) && class_exists('PC_Support_CPT') ) $this->_support_cpt_class = new PC_Support_CPT();
				}
            }
        }
		...
		/* Show simple debug output. */
		if( current_theme_supports( 'pc-custom-header' ) ) {
			PC_Hooks::pc_setup_custom_header_images(); // action hook
		}

		/* Support auto theme updates. */
		if( current_theme_supports( 'auto-update' ) ) {

            if( is_array( $_wp_theme_features['auto-update'] ) ) {
 
				/* Get custom post types. */
                $auto_update_setings = $_wp_theme_features['auto-update'][0];

				if( is_array($auto_update_setings) ) {
					new WPUpdatesThemeUpdater( $auto_update_setings[0], $auto_update_setings[1], basename(get_template_directory()) );
				}
            }
		}
		...
		/* Show theme activation message (via our 'theme_activated' callback function). */
...
//			wp_register_script( 'blog_facebook_social_button', 'http://connect.facebook.net/en_US/all.js#appId=144100152333902&amp;xfbml=1' );
	...
	/**
	 * Register and enqueue modernizr scripts.
	 *
	 * @since 0.1.0
	 */
    ...
    /**
     * Register and enqueue superfish scripts.
     *
     * @since 0.1.0
     */
	 ...
	 *
	 * @todo this and the callback reference should probably belong in the Portfolio CPT.
	 *
	 * @since 0.1.0
	 */
	...
	public function after_theme_activated() {
		...
		// @todo delete this code below in a future version of the framework
		// Check to see if legacy logo url is set. If so then check if the customizer logo url is set.
		// If so do nothing, otherwise copy across the legacy logo url and delete it from the options db.

		// If we're not using the 'legacy' uploader in theme options check if there is an existing url to copy to the customizer.
		if( !current_theme_supports( 'custom-logo' ) ) {

			$options = get_option( PC_OPTIONS_DB_NAME );

			if( isset($options['txt_logo_url']) ) {
				$customizer_logo_url = get_theme_mod( 'pc_txt_logo_url' );
				if( empty($customizer_logo_url) ) {
					set_theme_mod( 'pc_txt_logo_url', $options['txt_logo_url'] );
					// delete logo theme option
					unset($options['txt_logo_url']);
					update_option( PC_OPTIONS_DB_NAME, $options );

					// Copy the status of the hide tagline checkbox from theme options if set, to the theme customizer
					// Adding the hide tagline code here so it is only run once, when the logo url code is run, otherwise
					// it will run every time the theme is activated.
					if( isset($options['chk_hide_description']) && $options['chk_hide_description'] )
						set_theme_mod( 'chk_hide_description', 1 );
					else
						set_theme_mod( 'chk_hide_description', 0 );

					// delete tagline theme option
					unset($options['chk_hide_description']);
					update_option( PC_OPTIONS_DB_NAME, $options );
				}
			}
		}
