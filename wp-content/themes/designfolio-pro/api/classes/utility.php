...
class PC_Utility {
	 ...
	 * @todo Deprecate this function eventually as the logo uploader should be handled by the theme customizer for all themes.
	 *
	 * @since 0.1.0
	 */
	public static function pc_display_theme_logo($options) {
		...
		/* Make sure logo uses https protocol if current page is SSL. */
		$logo_url = $options[PC_LOGO_URL_OPTION_NAME];
		if( is_ssl() )
			$logo_url = str_replace("http", "https", $logo_url);

		// Show image logo if one defined
		if( current_theme_supports('custom-logo') && isset($options[ PC_LOGO_CHK_OPTION_NAME ]) && $options[ PC_LOGO_CHK_OPTION_NAME ] ) {
			if ( $options[ PC_LOGO_URL_OPTION_NAME ] != "" ) {
			?>
			<div id="site-logo"><a href="<?php echo get_home_url(); ?>" /><img src="<?php echo $logo_url; ?>" /></a></div>
			<?php
			}
			else {
	 ...
	 * Show logo in site header.
	 *
	 * This function replcaces pc_display_theme_logo() to display the site logo. It uses the theme customizer to handle the site logo.
	 *
	 * @since 0.1.0
	 */
	public static function pc_display_customizer_site_logo() {
		...
		/* Make sure logo uses https protocol if current page is SSL. */
		if( is_ssl() )
			$pc_txt_logo_url = str_replace("http", "https", $pc_txt_logo_url);

		// Show image logo if one defined
		if( !empty($pc_txt_logo_url) ) {
			?>
			<div id="site-logo"><a href="<?php echo get_home_url(); ?>" /><img src="<?php echo $pc_txt_logo_url; ?>" /></a></div>
			<?php
		}
		// Otherwise show title text instead of logo
		else {
	...
	public static function install_default_content($theme_options_url) {
		...
		//	require_once(get_template_directory().'/api/plugins/wordpress-importer/wordpress-importer.php');
		//}

		//if( !class_exists('WXR_Parser') ) {
		//	require_once(get_template_directory().'/api/plugins/wordpress-importer/parsers.php');
		//}

		//$file = get_template_directory().'/includes/misc/designfolio.wordpress.2013-03-11.xml';
		...
		//	$default_content = new WP_Import();
		
		//	$default_content->version = $import_data['version'];
		//	$default_content->get_authors_from_import( $import_data );
		//	$default_content->posts = $import_data['posts'];
		//	$default_content->terms = $import_data['terms'];
		//	$default_content->categories = $import_data['categories'];
		//	$default_content->tags = $import_data['tags'];
		//	$default_content->base_url = esc_url( $import_data['base_url'] );

		//	$default_content->process_posts();
			...
			//echo '</pre>';
		//} else {
		//	exit('Failed to open default content import file.');
		//}

		//return; // stop the normal default content doe from installing.
