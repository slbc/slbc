...
class PC_MetaBoxes {
	...
	public function theme_meta_box_init() {
		...
		if ( current_theme_supports('seo') ) {
			if ( defined('PC_SEO_SETTINGS_CHECKBOX') && isset($options[ PC_SEO_SETTINGS_CHECKBOX ]) && $options[ PC_SEO_SETTINGS_CHECKBOX ] ) {
		...
		if( function_exists( 'theme_meta_box_content_templates' ) ) {
			// @todo This option needs removing. Check all themes for code that uses this constant and remove it.
	...
	public function theme_widget_areas_save_meta_box($post_id) {
		...
		/* Just return if we're on a framework CPT page. */
		if( $typenow != 'post' && $typenow != 'page' )
			return;
		
		/* Don't need an isset() test as this meta data needs to save whether empty (no widgets) or not. */
		/* Save the meta box data as post meta, using the post ID as a unique prefix */

		// @todo might be able to delete the PC_THEME_NAME_SLUG.'_widget_areas_save' field (and all related code) if redundant, which I think it is.
	...
	public function theme_display_options_save_meta_box($post_id) {
		...
		if(isset($_POST[ PC_THEME_NAME_SLUG.'_column_layout' ]))
			update_post_meta($post_id, '_'.PC_THEME_NAME_SLUG.'_column_layout', esc_attr($_POST[ PC_THEME_NAME_SLUG.'_column_layout' ]));

		// Can't use the PC_THEME_NAME_SLUG.'_hide_title_header_tag' test here as the check box won't set $_POST variable if unchecked.
	 ...
	 *
	 * @todo function should be moved, as it is not strictly metabox related?
	 *
	 * @since 0.1.0
	 */
	public function pc_theme_title() {
		...
		$options = get_option( PC_OPTIONS_DB_NAME );
		if( current_theme_supports('seo') && defined('PC_SEO_SETTINGS_CHECKBOX') && isset($options[ PC_SEO_SETTINGS_CHECKBOX ]) && $options[ PC_SEO_SETTINGS_CHECKBOX ] ) {
			global $wp_query;
...
<?php
		...
		/* Just return if theme doesn't support SEO featue. */
		if ( !current_theme_supports('seo') )
			return;
		...
		$options = get_option( PC_OPTIONS_DB_NAME );
		if ( defined('PC_SEO_SETTINGS_CHECKBOX') && isset($options[ PC_SEO_SETTINGS_CHECKBOX ]) && $options[ PC_SEO_SETTINGS_CHECKBOX ] ) { // if this constant not set then this will always be false
			...
			/* Don't output empty meta description and keyword tags. */
			if( !empty($pc_theme_description) ) {
?><meta name="description" content="<?php echo $pc_theme_description; ?>" />
<?php
			}
			if( !empty($pc_theme_keywords) ) {
