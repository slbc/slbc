...
jQuery(document).ready(function ($) {
	 ...
	 */
	$('.wpfc_upload_file').change(function () {
		formfield = $(this).attr('name');
		$('#' + formfield + '_id').val("");
	});

	$('.wpfc_upload_button').live('click', function () {
		var buttonLabel;
		formfield = $(this).prev('input').attr('name');
		buttonLabel = 'Use as ' + $('label[for=' + formfield + ']').text();
		tb_show('', 'media-upload.php?post_id=' + $('#post_ID').val() + '&type=file&wpfc_force_send=true&wpfc_send_label=' + buttonLabel + '&TB_iframe=true');
		return false;
	});

	$('.wpfc_remove_file_button').live('click', function () {
    ...
    window.send_to_editor = function (html) {
		...
		if (formfield) {
			...
			$('#' + formfield).siblings('.wpfc_upload_status').slideDown().html(uploadStatus);
			tb_remove();
